...
public class EmprestimoController {
    ...
    public EmprestimoController(EmprestimoService emprestimoService, AlunoService alunoService, LivroService livroService) {
    ...
    @GetMapping
    public String listar(@RequestParam(name = "status", required = false)StatusEmprestimo status,
                         @RequestParam(name = "nomeAluno", required = false) String nomeAluno,
                         Model model) {
        if (nomeAluno != null && !nomeAluno.isBlank()) {
            model.addAttribute("emprestimos", emprestimoService.buscarPorNomeAluno(nomeAluno));
        } else {
            model.addAttribute("emprestimos",emprestimoService.listarTodos(status));
        }
        ...
        model.addAttribute("totalDevolucoesHoje", emprestimoService.contarDevolucoesHoje());
    ...
    public String salvar(@RequestParam("alunoId") Long alunoId,
        ...
        try {
            ...
            redirectAttributes.addFlashAttribute("mensagemSucesso", "Empréstimo realizado com secesso!");
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("mensagemErro", "Erro ao realizar empréstimo: " + e.getMessage());
        }
        return "redirect:/emprestimo";
    }

    @GetMapping("/renovar/{id}")
    public String renovar(@PathVariable("id") Long id, RedirectAttributes redirectAttributes) {
        try {
            emprestimoService.renovarEmprestimo(id);
            redirectAttributes.addFlashAttribute("mensagemSucesso", "Empréstimo renovado por mais 14 dias comsucesso!");
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("mensagemErro", "Erro ao renovar empréstimo: " + e.getMessage());
        }
    ...
    public String devolver(@PathVariable("id") Long id, RedirectAttributes redirectAttributes) {
        try {
            ...
            redirectAttributes.addFlashAttribute("mensagemSucesso", "Lvro devolvido com sucesso!");
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("mensagemErro", "Erro ao devolver livro: " + e.getMessage());
        }
        return "redirect:/empresimos";
    }
    ...
    public String excluir(@PathVariable("id") Long id, RedirectAttributes redirectAttributes) {
        try {
            ...
            redirectAttributes.addFlashAttribute("mensagemSucesso", "Emprétimo excludo com sucesso!");
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("mensagemErro", "Erro ao excluir empréstimo: " + e.getMessage());
        }
        return "redirect:/emprestimos";
    }
}
